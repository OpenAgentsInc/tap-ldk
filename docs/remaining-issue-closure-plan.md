...
`5bd5992ac7f7625f254e5df67eec66d085fe7c7d`, so later issue verification does
not fail against the older proof-ownership-only fork revision.

Path B reaches live funding but is not live-funded or live-settled. The current
#57/#81 gate reaches:
...
- integrated `litd` asset issuance and live asset-channel funding.

The gate still stops at `live_asset_channel_funding`. The current live blocker
is not peer readiness: Rust Lightning rejects the peer's initial 0-HTLC
simple-taproot commitment partial signature. The next required step is matching
Lightning Labs' Taproot Asset commitment output construction and sorting for
that initial funding commitment. Dynamic HTLC/change outputs are still needed
after funding verifies, and this issue set must remain open until both
directions observe post-settlement balances from real live Lightning Labs
funding/payment.
...
| Done | #78 Pin forked `ldk-node` to forked `rust-lightning` | `OpenAgentsInc/ldk-node` is pinned to `OpenAgentsInc/rust-lightning@5bd5992ac7f7625f254e5df67eec66d085fe7c7d`; `tap-ldk` consumes the OpenAgentsInc fork line and reports provenance. | Closed. |
...
| Done | #80 Wire asset messages and payment APIs | Implemented in `OpenAgentsInc/ldk-node@da05c714be061706806bc8757ee74b4709d5a8ef`, with litd-compatible Init feature cleanup and peer taproot feature reporting through `b750d91c92541529010259518eb1cc773dc5b5b8`; `tap-ldk` pins the latest revision and the live preflight reaches typed asset custom-message, asset-channel open, asset-payment APIs, Lightning Labs aux Init feature bits, and remote feature reporting. The fork advertises the Lightning Labs aux feature vector for no-op HTLCs and STXO; native STXO commitment semantics remain part of #81's dynamic-output work. | Closed. |
| 1 | #81 Fork-backed Lightning Labs settlement | Current live gate connects to `litd`, observes both taproot feature sets, issues an asset, and reaches live asset-channel funding. It fails when Rust Lightning rejects the peer's initial 0-HTLC simple-taproot commitment partial signature because local Taproot Asset commitment output construction/sorting does not match Lightning Labs. | Rust Lightning matches Lightning Labs' initial funding commitment output construction/sorting, then carries asset HTLC blobs into per-commitment asset state, derives dynamic aux leaves/output scripts, verifies `litd` signatures, and `tap-ldk` settles a live payment with observed balances. |
