...
live blocker is not peer readiness: Rust Lightning now strictly decodes the
live Taproot Asset HTLC blob and can carry HTLC aux leaves into output
construction, but it still verifies the `litd` commitment signature against a
transaction with static/plain simple-taproot HTLC and change outputs. The
latest fork persists and re-emits the live asset HTLC blob, so the remaining
gap is deriving the dynamic outputs from that asset state. `litd` signs
dynamically updated Taproot Asset HTLC and change outputs. This issue set must
remain open until Rust Lightning derives those dynamic aux
leaves/output scripts and both directions observe post-settlement balances
...
| 1 | #81 Fork-backed Lightning Labs settlement | Current live gate reaches active `litd` asset-channel funding and first asset HTLC. Rust Lightning validates, persists, and re-emits the live HTLC asset blob and has an HTLC aux-leaf output hook, then still fails commitment signature verification because it does not yet derive the dynamic Taproot Asset HTLC/change outputs `litd` signs. | Rust Lightning carries asset HTLC blobs into per-commitment asset state, derives dynamic aux leaves/output scripts, verifies `litd` signatures, and `tap-ldk` settles a live payment with observed balances. |
