...
`a626a77d951bbc069ce1c299a448d1bf3403bc0f`, so later issue verification does
...
#81 remains open because the post-success path still fails: after the native
claim/fulfill path, Lightning Labs force-closes with `invalid commitment`, and
the on-chain HTLC-success fallback/broadcast path still needs transcript and
witness cleanup. The current `rust-lightning` pin emits the zero-HTLC asset
commitment-sig blob (`Some([0])`) on the post-claim `commitment_signed`, so the
remaining mismatch is deeper than a missing no-HTLC asset signature TLV. Keep
this successful settlement transcript fixture-backed and port the remaining
bounded single-asset Lightning Labs `tapchannel`/`tapsend` commitment semantics
instead of making another local patch.
...
| Done | #78 Pin forked `ldk-node` to forked `rust-lightning` | `OpenAgentsInc/ldk-node` is pinned to `OpenAgentsInc/rust-lightning@a626a77d951bbc069ce1c299a448d1bf3403bc0f`; `tap-ldk` consumes the OpenAgentsInc fork line and reports provenance. | Closed. |
