...
`7bc73cf1ef7e2381c0562d61bfcdce9a18579cae`, so later issue verification does
...
The current gate stops at `live_asset_channel_payment_settlement`: the live
asset keysend stays `IN_FLIGHT` after Rust Lightning accepts the peer
`commitment_signed`. The current fork pin stores the proof-derived
single-asset channel template, verifies the peer HTLC signature as BIP340
Schnorr, captures the earlier failing transcript as a regression fixture, adds
the Lightning Labs second-level virtual `lock_time`/`relative_lock_time`
asset-leaf fields, persists the full counterparty commitment through monitor
update `1`, and derives exact previous-output-bound second-level HTLC aux
leaves before our outgoing signature. The latest completed live run before
that exact-leaf pin got past monitor completion and message release, then
`litd` rejected our outgoing HTLC signature as `invalid_htlc_sig`. #81 remains
open until the exact-leaf rerun settles, receiver claim, HTLC
witness/control-block path, and post-settlement balances are all observed from
real live Lightning Labs funding/payment.

The next #81 step is to rerun the live path against `litd` with the current
`rust-lightning` and `ldk-node` pins. If the outgoing signature still fails or
a later rerun exposes another signature or force-close transcript delta, keep
the captured transcript fixture-backed and port the remaining bounded
single-asset Lightning Labs `tapchannel`/`tapsend` allocation semantics instead
of making another local patch.
...
| Done | #78 Pin forked `ldk-node` to forked `rust-lightning` | `OpenAgentsInc/ldk-node` is pinned to `OpenAgentsInc/rust-lightning@7bc73cf1ef7e2381c0562d61bfcdce9a18579cae`; `tap-ldk` consumes the OpenAgentsInc fork line and reports provenance. | Closed. |
