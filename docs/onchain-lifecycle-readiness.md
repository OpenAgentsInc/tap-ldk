...
and final sweep cases. It does not yet have a single typed report that says
which on-chain lifecycle events were explained, which were refused, which were
only bounded simulations, and which remain live-regtest or chain-watcher work.
...
This is still not the live chain watcher. The bounded gate should make the
current proof-ownership and close/recovery evidence explicit, deterministic,
and testable. The production work after this gate is to drive the same event
...
Remaining after the bounded gate:
