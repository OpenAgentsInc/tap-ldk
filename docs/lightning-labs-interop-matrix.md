...
| Proof-engine verification | `formal/tla/proof_validation`; `crates/tap-ldk-core/tests/proof_replay_properties.rs`; `fuzz/fuzz_targets`; `scripts/proof-engine-check.sh`; `cloudbuild.yaml` | Proof replay, negative vectors, wallet/funding/commitment/close/recovery replay gates, bounded anchor policy, bounded on-chain lifecycle reporting, bounded chain/sweeper observation reporting, live-regtest callback binding, formal checks, Rust property tests, fuzz target compile/smoke paths, and optional Kani harnesses are now covered by the local wrapper and Google Cloud Build runner path. | Implemented for bounded hardening suite |
