...
| Proof-engine verification | `formal/tla/proof_validation`; `crates/tap-ldk-core/tests/proof_replay_properties.rs`; `fuzz/fuzz_targets`; `scripts/proof-engine-check.sh`; `.github/workflows/proof-engine.yml` | Proof replay, negative vectors, wallet/funding/commitment/close/recovery replay gates, bounded anchor policy, formal checks, Rust property tests, fuzz target compile/smoke paths, and optional Kani harnesses are now covered by local wrapper and CI workflow surfaces. | Implemented for bounded hardening suite |
