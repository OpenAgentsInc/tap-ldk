...
1. #77 through #81: create and wire `OpenAgentsInc/ldk-node` so the live node
   runtime uses the OpenAgentsInc `rust-lightning` fork and exposes asset
   channel config, messages, and payment APIs.
...
- `tap-ldk` builds sender-side artifacts for the `tap-ldk` pays Lightning Labs
  direction and reaches the connected integrated `litd` peer through upstream
  `ldk-node`, but does not yet run fork-backed asset-channel funding/payment
