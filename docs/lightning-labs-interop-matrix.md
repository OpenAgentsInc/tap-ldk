...
| Force close | `tapchannel/aux_sweeper.go`; `itest/custom_channels/force_close_test.go`; `itest/custom_channels/htlc_force_close_test.go`; `docs/onchain-lifecycle-readiness.md` | Bounded proof-ownership recovery records now exist for commitment, second-level HTLC, and final sweep paths, BTC-only sweep state is refused as asset recovery, and `onchain-lifecycle.json` consolidates those records with close/restart evidence. Live daemon-backed resolver/sweeper interop remains open. | Partially implemented |
| Proof-engine verification | `formal/tla/proof_validation`; `crates/tap-ldk-core/tests/proof_replay_properties.rs`; `fuzz/fuzz_targets`; `scripts/proof-engine-check.sh`; `.github/workflows/proof-engine.yml` | Proof replay, negative vectors, wallet/funding/commitment/close/recovery replay gates, bounded anchor policy, bounded on-chain lifecycle reporting, formal checks, Rust property tests, fuzz target compile/smoke paths, and optional Kani harnesses are now covered by local wrapper and CI workflow surfaces. | Implemented for bounded hardening suite |
