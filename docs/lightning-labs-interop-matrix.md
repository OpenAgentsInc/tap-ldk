...
| Payment direction: Lightning Labs pays `tap-ldk` | `itest/custom_channels/core_test.go`; `tapchannel/aux_invoice_manager.go`; `docs/lightning-labs-incoming-payment.md` | Receiver-side Track B artifacts are built and persisted, and the live integrated `litd` keysend now settles into native LDK with durable receiver balance. The remaining #81 blocker is the post-success zero-HTLC commitment partial-signature mismatch plus force-close Taproot control-block verification; #58 still needs the full named reverse-direction acceptance flow and restart proof. | Partially implemented |
...
- `tap-ldk` builds receiver-side artifacts for the Lightning Labs pays
  ...
  partial-signature mismatch and force-close Taproot control-block path remain
  open.
