...
simple-taproot plus Taproot Asset channel negotiation flags.
...
provenance, and opt-in asset-channel negotiation config. Issue #80 still needs
to expose proof, funding, RFQ, quote, and asset HTLC APIs through `ldk-node`;
#81 then replaces this pre-settlement gate with live asset-channel
funding/payment against the independent `litd` counterparty. The current #57
report treats this as a readiness gate, not as live settlement.
