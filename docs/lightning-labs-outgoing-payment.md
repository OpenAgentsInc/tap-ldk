# `tap-ldk` to `litd` Outgoing Payment

`tap-ldk` now builds the sender-side artifacts for the first Track B payment
direction: a native `tap-ldk` wallet paying a `lnd`/`tapd`/`litd` counterparty. The
smoke uses the imported Lightning Labs software funding and commitment fixtures,
constructs the RFQ-bound invoice path, emits taproot-assets RFQ request/accept
...
`litd` to native direction, then has native LDK send the asset back to
...
- Builds and validates taproot-assets RFQ request/accept payloads.
...
- Queries the current `tapd` asset balance by asset ID when a
  live counterparty is available.
- Starts the integrated `litd` counterparty with LND,
...
#81, #57, #58, #59, and #60 are complete live/proof/reporting regression
gates. Keep `./scripts/live-lightning-labs-outgoing-payment.sh` and the live
tapd proof binding path green before closing the Path B epic.
