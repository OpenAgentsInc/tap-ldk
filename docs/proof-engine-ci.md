...
and the native wallet-to-wallet demo. Optional tools are visible: TLA+ skips
when no `tlc` or `TLA_TOOLS_JAR` is available, fuzz smoke skips when
`cargo-fuzz` is missing, and Kani skips when `cargo kani` is missing.
