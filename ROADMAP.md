...
- Path B has fixture/RFQ/payment checks, `tapd` proof binding, local peer
  ...
  P2WSH isolation, and fixes the BOLT
  ...
  until the remaining cooperative-close live proof and splice-boundary gaps are
  fixed. The
  detailed audits in
...
- `tap-ldk` is pinned to the OpenAgentsInc `rust-lightning` fork at
  `7150b421954d655d8e1a61612639f6987388a25a` and the OpenAgentsInc
  `ldk-node` fork at `766104066e8813e0108a80c98b98f2026a933d20`. BOLT
...
- Current open work includes #81, #57, #58, #59, #60, #61, #71, and #19. Issue
  ...
  #82, with focused issues #89 through #90 still open before #61/#71 can
  close. The
  ...
  tracked as #88. Issue #57 is still the true native
...
| Done | #80 | Wire Taproot Asset messages and payment APIs through `ldk-node` | Implemented in `OpenAgentsInc/ldk-node` at `da05c714be061706806bc8757ee74b4709d5a8ef`, with live-feature negotiation fixes, the current rust-lightning HTLC aux-leaf derivation pin, and proof-derived channel-template binding carried through `766104066e8813e0108a80c98b98f2026a933d20`; `tap-ldk` pins the latest revision and the live preflight reaches typed asset custom-message, asset-channel open, asset-payment APIs, Lightning Labs aux Init feature bits, and remote taproot feature reporting. The fork now advertises Lightning Labs no-op HTLC aux support and does not advertise STXO until native STXO commitment leaves are implemented. | Closed. |
| Done | #85 | Enforce private-only simple-taproot channels | Implemented in `OpenAgentsInc/rust-lightning@7150b421954d655d8e1a61612639f6987388a25a` and carried through `OpenAgentsInc/ldk-node@766104066e8813e0108a80c98b98f2026a933d20`: outbound simple-taproot and Taproot Asset opens clear `announce_channel`, inbound public simple-taproot/Taproot Asset opens fail closed, and legacy public BTC channel behavior remains unchanged. | Closed. |
| Done | #86 | Fail missing simple-taproot open/accept nonces immediately | Implemented in `OpenAgentsInc/rust-lightning@7150b421954d655d8e1a61612639f6987388a25a` and carried through `OpenAgentsInc/ldk-node@766104066e8813e0108a80c98b98f2026a933d20`: simple-taproot and Taproot Asset `open_channel`/`accept_channel` now fail before state advances when type-4 `next_local_nonce` is missing, while legacy channels can still omit the TLV. | Closed. |
| Done | #87 | Make type-22 RAA/reestablish nonce maps authoritative | Implemented in `OpenAgentsInc/rust-lightning@7150b421954d655d8e1a61612639f6987388a25a` and carried through `OpenAgentsInc/ldk-node@766104066e8813e0108a80c98b98f2026a933d20`: RAA and reestablish send only type-22 nonce maps, inbound scalar fallback is rejected once simple-taproot funding exists, and retransmitted partial signatures are keyed by fresh nonce-map material. | Closed. |
| Done | #88 | BTC-only simple-taproot conformance gate | Implemented in `OpenAgentsInc/rust-lightning@7150b421954d655d8e1a61612639f6987388a25a` and carried through `OpenAgentsInc/ldk-node@766104066e8813e0108a80c98b98f2026a933d20`: the gate opens a BTC-only simple-taproot channel, verifies P2TR funding, pays in both directions across reconnect/reestablish, covers functional cooperative close, force-closes with a one-element key-path funding witness, and proves legacy P2WSH channels remain unaffected. Run `./scripts/check-btc-simple-taproot-conformance.sh`. | Closed. |
...
| 6 | #61 | BOLT simple taproot channels in `rust-lightning` epic | Fork issues #62 through #70 and #75 are implemented and pinned, with vector/lifecycle smoke coverage. #88 now proves the BTC-only open/pay/reestablish/cooperative-close/force-close base with legacy-channel isolation. | BTC-only simple-taproot LDK channels open, pay, reestablish, cooperatively close, force-close, the BOLT spec-compliance tracker closes, and legacy channels remain unaffected. |
