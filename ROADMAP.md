...
- Path B has fixture/RFQ/payment checks, `tapd` proof binding, local peer
  ...
  an asset, and reaches live asset-channel funding. #81 remains open because
  Rust Lightning currently rejects the peer's initial 0-HTLC simple-taproot
  commitment partial signature. The next #81 step is matching Lightning Labs'
  Taproot Asset commitment output construction and sorting for the funding
  commitment; HTLC/payment settlement and observed balance recording come after
  that funding signature path verifies.
...
| 1 | #81 | Use fork-backed `ldk-node` for live Lightning Labs settlement | The live harness now connects to `litd`, observes both taproot feature sets, issues an asset, and reaches live asset-channel funding. The current live trace stops when Rust Lightning rejects the peer's initial 0-HTLC simple-taproot commitment partial signature; LDK must match Lightning Labs' Taproot Asset commitment output construction and sorting before the funding ack can complete. The earlier CSV split is fixed: zero CSV remains for asset allocation/script-key derivation, while the actual commitment output uses channel CSV. | The live Path B scripts settle an asset payment over fork-backed `ldk-node` and record post-settlement balances. |
