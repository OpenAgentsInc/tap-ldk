...
- Path B has fixture/RFQ/payment checks, `tapd` proof binding, local peer
  ...
  live zero-HTLC post-claim regression fixture. The latest live rerun no longer
  logs the post-claim partial-signature failure, invalid-commitment failure, or
  invalid Taproot control-block failure. #81 stays open until the force-close
  witness path is fixture-backed and live-proven. The current pin also fixes
  the BOLT simple-taproot audit's legacy signature-field zeroing/rejection gap
  for funding and commitment messages, but the fork is still not spec complete
  until the remaining force-close and broader conformance gaps are fixed. The
  detailed audits in
  ...
  file-level map for the remaining #81 force-close work. Broader
...
- `tap-ldk` is pinned to the OpenAgentsInc `rust-lightning` fork at
  `90212e54066a35ad982b338e7c2c152bf4fe0b0b` and the OpenAgentsInc
  `ldk-node` fork at `3264d96ee6dcbd37cec24473eac5982b1678a560`. BOLT
...
- Current open work includes #81, #57, #58, #59, #60, #61, #71, and #19. Issue
  #81 now focuses on the remaining force-close Taproot control-block/witness
  proof for the live settlement gate. The post-success zero-HTLC commitment
  partial-signature mismatch is fixed and tracked as #83. The remaining BOLT
  simple-taproot audit items are tracked under #82, with focused issues #84
  through #90, before #61/#71 can close. Issue #57 is still the true native
